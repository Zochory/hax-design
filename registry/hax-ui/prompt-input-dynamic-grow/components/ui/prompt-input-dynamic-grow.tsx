...
const OptionsMenu = memo(({ 
  ...
  return (
    <div className="absolute top-full left-0 mt-1 bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden z-30 min-w-[120px]">
      <ul className="py-1">
        {menuOptions.map((option) => (
          <li
            key={option}
            className={`px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer text-sm font-medium`}
...
const OptionTag = memo(({ 
  ...
  >
    ...
    <button
      ...
      className={`h-4 w-4 flex items-center justify-center rounded-full hover:opacity-70`}
...
export function PromptInputDynamicGrow({
    ...
    <ChatInputContext.Provider value={contextValue}>
      ...
      >
        <div
          ...
          onMouseEnter={(e) => {
            e.currentTarget.style.backgroundColor = `rgba(255, 255, 255, ${backgroundOpacity + 0.05})`;
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.backgroundColor = `rgba(255, 255, 255, ${backgroundOpacity})`;
          }}
        >
          {/* Visual effects */}
          <GlowEffects 
